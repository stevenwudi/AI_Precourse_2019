...
class Game:
    ...
    def update(self):
        ...
        track_pos_current = [bbox[0] + bbox[2]/2., bbox[1] + bbox[3]/2.]
        ...
        self.bird.flap_up_velocity = abs(vertical_ratio) / jump_vertical_ratio * 1
        ...
        right = min(img.shape[0], int(bbox[2]))
        top = max(0, int(bbox[1]))
        bottom = min(img.shape[1], int(bbox[3]))
            ...
            while self.cap.isOpened():
                ...
                if not track_flag:
                    ...
                    if count_defects > hand_convex_number:
                        ...
                        track_flag = True
                        ok, bbox = self.tracker.update(img)
                        self.track_pos_prev = [bbox[0] + bbox[2]/2., bbox[1] + bbox[3]/2.]
